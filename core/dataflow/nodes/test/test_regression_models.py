...
class TestLinearRegression(hut.TestCase):
    ...
    def test_generate_input_data(self) -> None:
        ...
        def _compute_df_signature(df: pd.DataFrame) -> str:
            ...
            return "\n".join(txt)
        self.assert_equal(_compute_df_signature(data), _compute_df_signature(data2))

    @pytest.mark.skip(reason="This test fails on some computers due to AmpTask1649")
    def test0(self) -> None:
        """
    ...
    def _get_data_from_disk(self) -> pd.DataFrame:
        """
        Read the data generated by `test_generate_input_data()` through `_get_data()`.
        """
        file_name = self._get_test_data_file_name()
