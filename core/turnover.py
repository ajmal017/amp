...
def compute_turnover(df: pd.DataFrame) -> float:
    """
...
def maximize_weight_entropy(
    ...
    return _optimize_weights(
        df,
        turnover,
        lambda x: -1 * coentrop.compute_entropy(pd.Series(np.sqrt(x)), 1),
    )
...
def compute_distance_to_isoturnover_plane(
    ...
    """
    Compute distance of squared weights to isoturnover plane of squared weights.
    """
    hdbg.dassert_in("weight", df.columns)
    weight_sq = np.square(df["weight"])
...
def project_onto_isoturnover_plane(
    ...
    """
    Orthogonally project weights onto isoturnover plane in squared weight space.
